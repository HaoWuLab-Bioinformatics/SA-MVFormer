...
class Logger(object):
    def __init__(self, runs, info=None, dataset_name=None, total_runs=None):
        self.info = info
...
def load_model(args, model, optimizer, run):
    ...
    if getattr(args, 'cpu', False) or not torch.cuda.is_available():
        ...
        selected_device_index = getattr(args, 'device', 0)
        map_location = f'cuda:{selected_device_index}'
...
def save_result(args, results):
    ...
    hyperparams = []
    hyperparams.append(f"method: {args.method}")
    hyperparams.append(f"hidden_channels: {args.hidden_channels}")
    hyperparams.append(f"local_layers: {args.local_layers}")
    hyperparams.append(f"global_layers: {args.global_layers}")
    hyperparams.append(f"num_heads: {args.num_heads}")
    hyperparams.append(f"beta: {args.beta}")
    hyperparams.append(f"pre_ln: {args.pre_ln}")
    hyperparams.append(f"lr: {args.lr}")
    hyperparams.append(f"weight_decay: {args.weight_decay}")
    hyperparams.append(f"in_dropout: {args.in_dropout}")
    hyperparams.append(f"dropout: {args.dropout}")
    hyperparams.append(f"global_dropout: {args.global_dropout}")
    hyperparams.append(f"local_epochs: {args.local_epochs}")
    hyperparams.append(f"global_epochs: {args.global_epochs}")
    hyperparams.append(f"runs: {args.runs}")
    hyperparams.append(f"seed: {args.seed}")

    with open(f"{filename}", 'a+') as write_obj:
        write_obj.write(f"{args.method} {args.dropout} {args.lr} {results.mean():.2f} ± {results.std():.2f}")
        write_obj.write(f" | hyperparams: {' | '.join(hyperparams)}\n")
